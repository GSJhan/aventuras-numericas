...
async function loadUser() {
  ...
  document.getElementById('displayLevel').textContent = '🏆 ' + calcLevel();
...
function initGame() {
  ...
  document.getElementById('goToCalculatorBtn').onclick = function() {
    document.getElementById('gameSection').classList.add('hidden');
    document.getElementById('calculatorSection').classList.remove('hidden');
  };

  document.getElementById('backToGameMenu').onclick = function() {
...
function generateQuestion() {
  ...
  if (difficulty === 'facil') {
    ...
    if (t === 0) {
      ...
      hint = 'Leyes de exponentes';
    }
  } else {
    var t = rnd(0, 3);
    if (t === 0) {
      var r1 = rnd(1, 8), r2 = rnd(1, 8);
      q = 'Suma de raíces de x²' + (r1 + r2 >= 0 ? '+' : '') + (r1 + r2) + 'x + ' + (r1 * r2) + ' = 0';
      correct = r1 + r2;
      hint = 'Suma de raíces = -b/a';
    } else if (t === 1) {
      var r1 = rnd(1, 7), r2 = rnd(1, 7);
      q = 'Producto de raíces de x²' + (r1 + r2 >= 0 ? '+' : '') + (r1 + r2) + 'x + ' + (r1 * r2) + ' = 0';
      correct = r1 * r2;
...
function showQuestion() {
  ...
  while (opts.length < 4) {
    opts.push(correct + opts.length * (difficulty === 'experto' ? 7 : 3));
  }
  
  opts.sort(function() { return Math.random() - 0.5; });

  var hintHtml = hint ? '<div style="font-size:13px;color:#9b59ff;margin-bottom:15px;">💡 ' + hint + '</div>' : '';
  var html = '<div class="question-box">' + hintHtml;
  ...
  for (var j = 0; j < opBtns.length; j++) {
    opBtns[j].addEventListener('click', function() {
      ...
      if (Number(this.dataset.val) === correct) {
        ...
        user.estadisticas.totalPreguntas++;
        user.estadisticas.respuestasCorrectas++;
        if (streak > user.estadisticas.rachaMaxima) user.estadisticas.rachaMaxima = streak;
...
function solveEquation() {
  ...
  html += '<h3 style="color:#4c90ff;">📐 Resolución paso a paso</h3>';
  html += '<p><strong>Ecuación:</strong> x² ' + (b >= 0 ? '+' : '') + b + 'x ' + (c >= 0 ? '+' : '') + c + ' = 0</p>';
  html += '<p>a = 1, b = ' + b + ', c = ' + c + '</p>';
  html += '<hr>';
  html += '<p><strong>Paso 1: Calcular discriminante</strong></p>';
  html += '<p>Δ = b² - 4ac</p>';
  html += '<p>Δ = (' + b + ')² - 4(1)(' + c + ')</p>';
  html += '<p>Δ = ' + (b * b) + ' - ' + (4 * c) + ' = <strong>' + delta + '</strong></p>';
  
  if (delta < 0) {
    html += '<p style="color:#ff4d6d;">❌ La ecuación no tiene soluciones reales (Δ < 0)</p>';
  } else if (delta === 0) {
    var x = (-b / 2).toFixed(2);
    html += '<p><strong>Paso 2: Solución única (raíz doble)</strong></p>';
    html += '<p>x = -b/(2a) = -(' + b + ')/2 = ' + x + '</p>';
    html += '<p style="color:#4cff90;">✅ Solución: x = ' + x + '</p>';
  } else {
    var sqrtDelta = Math.sqrt(delta).toFixed(4);
    var x1 = ((-b + Math.sqrt(delta)) / 2).toFixed(2);
    var x2 = ((-b - Math.sqrt(delta)) / 2).toFixed(2);
    html += '<p><strong>Paso 2: Calcular raíces</strong></p>';
    html += '<p>√Δ = ' + sqrtDelta + '</p>';
    html += '<p>x₁ = (-b + √Δ)/(2a) = ' + x1 + '</p>';
    html += '<p>x₂ = (-b - √Δ)/(2a) = ' + x2 + '</p>';
