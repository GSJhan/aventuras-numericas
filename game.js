...
document.getElementById('solveBtn').onclick = function() {
  ...
  if (delta < 0) {
    ...
    html += '<p style="color:#aaa;font-size:13px">Solo existen soluciones complejas (imaginarias).</p>';
    ...
    html += '<p style="color:#4cff90"><strong>Paso 2 — Analizar la Discriminante:</strong></p>';
    html += '<p style="color:#aaa;font-size:13px">Como Δ = ' + delta + ' es mayor que 0,</p>';
    ...
    html += '<p style="color:#aaa;font-size:13px">x₁ = (-b + √Δ) / 2a</p>';
    html += '<p style="color:#aaa;font-size:13px">x₁ = (-(' + b + ') + ' + sqrtDelta.toFixed(4) + ') / 2×1</p>';
    html += '<p style="color:#aaa;font-size:13px">x₁ = (' + (-b) + ' + ' + sqrtDelta.toFixed(4) + ') / 2</p>';
    html += '<p style="color:#aaa;font-size:13px">x₁ = ' + ((-b) + sqrtDelta).toFixed(4) + ' / 2</p>';
    ...
    html += '<hr style="border-color:rgba(76,144,255,0.2);margin:10px 0"/>';
    html += '<p><strong>Paso 5 — Verificación:</strong></p>';
    var v1 = (Math.pow(Number(x1),2) + b*Number(x1) + c).toFixed(2);
    var v2 = (Math.pow(Number(x2),2) + b*Number(x2) + c).toFixed(2);
    html += '<p style="color:#aaa;font-size:13px">Sustituyendo x₁ = ' + x1 + ': (' + x1 + ')² + (' + b + ')(' + x1 + ') + (' + c + ') = ' + v1 + ' ≈ 0 ✅</p>';
    html += '<p style="color:#aaa;font-size:13px">Sustituyendo x₂ = ' + x2 + ': (' + x2 + ')² + (' + b + ')(' + x2 + ') + (' + c + ') = ' + v2 + ' ≈ 0 ✅</p>';
  }

  html += '</div>';

  // --- Datos para el canvas ---
  ...
  html += '<p style="color:#4c90ff;font-family:Orbitron,monospace;margin:18px 0 4px;font-size:13px">📊 Gráfica de la parábola <span style="font-size:10px;color:#666">— rueda del mouse para zoom · arrastra para mover · pellizca en tablet</span></p>';
  ...
  function toSX(x, minX, maxX) {
    return padL + ((x - minX) / (maxX - minX)) * graphW * scale + offsetX;
  }
  function toSY(y, minY, maxY) {
    return padT + graphH - ((y - minY) / (maxY - minY)) * graphH * scale + offsetY;
  }

  function drawGraph() {
    var minX = baseMinX, maxX = baseMaxX;
    var minY = baseMinY, maxY = baseMaxY;
    ...
    var zY = toSY(0, minY, maxY);
    var zX = toSX(0, minX, maxX);
    ...
    ctx.lineWidth = 1;
    var step = 1;
    for (var gxi = Math.floor(minX - 15); gxi <= Math.ceil(maxX + 15); gxi += step) {
      var lx = toSX(gxi, minX, maxX);
      if (lx > padL && lx < W - padR) {
        ctx.beginPath(); ctx.moveTo(lx, padT); ctx.lineTo(lx, H - padB); ctx.stroke();
      }
    }
    for (var gyi = Math.floor(minY - 15); gyi <= Math.ceil(maxY + 15); gyi += step) {
    ...
    ctx.fillText('x', W - padR + 4, czY + 4);
    ctx.fillText('y', czX + 5, padT + 4);
    ...
    for (var tx = Math.floor(minX - 10); tx <= Math.ceil(maxX + 10); tx++) {
      ...
      if (txp > padL + 6 && txp < W - padR - 6) {
        ctx.strokeStyle = 'rgba(255,255,255,0.4)'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.moveTo(txp, czY - 4); ctx.lineTo(txp, czY + 4); ctx.stroke();
        ctx.fillText(tx, txp - 4, czY + 15);
      }
    }
    ...
    for (var ty = Math.floor(minY - 10); ty <= Math.ceil(maxY + 10); ty++) {
      ...
      if (typ > padT + 6 && typ < H - padB - 6) {
        ctx.strokeStyle = 'rgba(255,255,255,0.4)'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.moveTo(czX - 4, typ); ctx.lineTo(czX + 4, typ); ctx.stroke();
        ctx.fillText(ty, czX - (ty < 0 ? 28 : 22), typ + 4);
      }
    }
    // origen
    ctx.fillStyle = 'rgba(255,255,255,0.4)';
    ...
    grad.addColorStop(0,   '#9b59ff');
    grad.addColorStop(0.5, '#4c90ff');
    ...
    for (var xp = minX - 8; xp <= maxX + 8; xp += 0.03) {
      var yp = xp*xp + b*xp + c;
      var sxp = toSX(xp, minX, maxX);
      var syp = toSY(yp, minY, maxY);
      if (first) { ctx.moveTo(sxp, syp); first = false; }
    ...
    // vértice
    var vsx = toSX(vertexX, minX, maxX);
    var vsy = toSY(vertexY, minY, maxY);
    ...
    if (vlx > W - 90) vlx = vsx - 88;
    if (vly < padT + 12) vly = vsy + 20;
    ctx.fillText('V(' + vertexX.toFixed(2) + ', ' + vertexY.toFixed(2) + ')', vlx, vly);

    // raíces
    if (delta >= 0 && rx1 !== null) {
      ctx.shadowColor = '#4cff90'; ctx.shadowBlur = 12;
      if (Math.abs(rx1 - rx2) < 0.01) {
        var rsx = toSX(rx1, minX, maxX);
        ctx.fillStyle = '#ffd700';
        ctx.beginPath(); ctx.arc(rsx, czY, 7, 0, Math.PI*2); ctx.fill();
        ...
        var r1sx = toSX(rx1, minX, maxX);
        var r2sx = toSX(rx2, minX, maxX);
        ctx.fillStyle = '#4cff90';
        ctx.beginPath(); ctx.arc(r1sx, czY, 7, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(r1sx, czY, 3, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = '#4cff90';
        ctx.beginPath(); ctx.arc(r2sx, czY, 7, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(r2sx, czY, 3, 0, Math.PI*2); ctx.fill();
        ctx.shadowBlur = 0;
        ctx.fillStyle = '#4cff90';
        ctx.font = 'bold 11px Orbitron, monospace';
        var r1lx = r1sx + 8, r1ly = czY - 14;
        if (r1lx > W - 70) r1lx = r1sx - 68;
        ctx.fillText('x₁=' + rx1.toFixed(2), r1lx, r1ly);
        var r2lx = r2sx + 8, r2ly = czY - 14;
        if (r2lx > W - 70) r2lx = r2sx - 68;
        ctx.fillText('x₂=' + rx2.toFixed(2), r2lx, r2ly);
      }
    }

    // fórmula abajo
    ctx.fillStyle = 'rgba(76,144,255,0.45)';
  ...
  wrapper.addEventListener('touchmove', function(e) {
    if (e.touches.length === 2) {
      ...
      if (lastPinchDist > 0) {
        scale = Math.max(0.2, Math.min(10, scale * (dist / lastPinchDist)));
        drawGraph();
      }
      lastPinchDist = dist;
...
function generateQuestion() {
  ...
  if (difficulty === 'facil') {
    var t = rnd(0,1);
    if (t === 0) { var a=rnd(1,20),b=rnd(1,20); q=a+' + '+b; correct=a+b; }
    else { var a=rnd(10,30),b=rnd(1,10); q=a+' - '+b; correct=a-b; }
  } else if (difficulty === 'normal') {
    var t = rnd(0,1);
    if (t === 0) { var a=rnd(2,12),b=rnd(2,10); q=a+' × '+b; correct=a*b; }
    else { var a=rnd(10,50),b=rnd(1,30); q=a+' - '+b; correct=a-b; }
  } else if (difficulty === 'dificil') {
    var t = rnd(0,2);
    if (t === 0) { var a=rnd(5,20),b=rnd(5,15); q=a+' × '+b; correct=a*b; }
    else if (t === 1) { var a=rnd(2,10); q=a+'²'; correct=a*a; }
    else { var a=rnd(2,8),b=rnd(2,5),c=rnd(1,10); q=a+' × '+b+' + '+c; correct=a*b+c; }
  } else {
    var t = rnd(0,2);
    if (t === 0) { var a=rnd(2,9),b=rnd(2,4); q=a+'^'+b; correct=Math.pow(a,b); }
    else if (t === 1) { var a=rnd(2,15),b=rnd(2,9),c=rnd(1,20); q=a+' × '+b+' - '+c; correct=a*b-c; }
    else { var a=rnd(2,10); q=a+'³'; correct=a*a*a; }
  }
  return { q: q, correct: correct };
}

function showQuestion() {
  var qObj = generateQuestion();
  var q = qObj.q, correct = qObj.correct;
  var spread = difficulty === 'facil' ? 10 : difficulty === 'normal' ? 15 : difficulty === 'dificil' ? 20 : 50;
  var opts = [correct];
  var attempts = 0;
  while (opts.length < 4 && attempts < 200) {
    var v = correct + rnd(-spread, spread);
    if (opts.indexOf(v) === -1 && v !== correct) opts.push(v);
    attempts++;
  }
  ...
  html += '<div class="question-text">' + q + ' = ?</div>';
  ...
  for (var j = 0; j < opBtns.length; j++) {
    opBtns[j].addEventListener('click', function() {
      ...
      if (Number(this.dataset.val) === correct) {
        this.style.background = '#1a5c2a';
        this.style.borderColor = '#4cff90';
        streak++;
        quizPoints++;
        ...
        document.getElementById('quizStats').innerHTML =
          '<span class="correct">✅ ¡Correcto! +' + xpR[difficulty] + '⭐ +' + coinR[difficulty] + '💰</span><br>' +
          'Racha: ' + streak + ' | Puntos: ' + quizPoints + ' | Nivel: ' + calcLevel();
        ...
        for (var m = 0; m < allBtns.length; m++) {
          if (Number(allBtns[m].dataset.val) === correct) {
            allBtns[m].style.background = '#1a5c2a';
            allBtns[m].style.borderColor = '#4cff90';
          }
        }
        document.getElementById('quizStats').innerHTML =
          '<span class="wrong">❌ Incorrecto. Era: <strong>' + correct + '</strong></span><br>' +
          'Racha perdida | Puntos: ' + quizPoints;
