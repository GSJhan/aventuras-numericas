...
function initMenu() {
  // Manejo de navegación por data-page para coincidir con el HTML original
  ...
  // Botón Verificar Infinito
  const infinitySolveBtn = document.getElementById('infinitySolveBtn');
  if (infinitySolveBtn) {
    infinitySolveBtn.onclick = checkInfinity;
  }
}

function showSection(page) {
  // Ocultar todas las secciones
  const sections = ['shopSection', 'avatarSection', 'skillsSection', 'duelsSection', 'logrosSection', 'rankingSection', 'infinitoSection'];
  sections.forEach(id => {
...
function generateProblem(difficulty) {
  ...
  switch(difficulty) {
    ...
    case 'normal':
      ...
      a = window.rnd(5, 20); b = window.rnd(5, 20);
      if (op2 === 'x') { question = a + " × " + b; result = a * b; }
      else { question = (a * b) + " ÷ " + b; result = a; }
      break;
    case 'dificil':
      const op3 = Math.random() > 0.5 ? 'p' : 'r';
      if (op3 === 'p') { a = window.rnd(2, 10); b = window.rnd(2, 4); question = a + "^" + b; result = Math.pow(a, b); }
      else { a = window.rnd(2, 10); question = "√" + (a * a); result = a; }
      break;
    case 'extremo':
...
function checkStreakAchievements() {
  if (!user.logros) user.logros = {};
  const streakAchievements = [{ id: 'streak_3', value: 3 }, { id: 'streak_5', value: 5 }, { id: 'streak_10', value: 10 }, { id: 'streak_25', value: 25 }, { id: 'streak_50', value: 50 }, { id: 'streak_100', value: 100 }, { id: 'streak_250', value: 250 }, { id: 'streak_500', value: 500 }];
...
function showAchievementNotification(achievementId) {
  const achievementNames = { 'streak_3': '🔥 Racha x3', 'streak_5': '🔥🔥 Racha x5', 'streak_10': '🔥🔥🔥 Racha x10', 'streak_25': '🌪️ Racha x25', 'streak_50': '⚡ Racha x50', 'streak_100': '💥 Racha x100', 'streak_250': '🌟 Racha x250', 'streak_500': '👑 Racha x500' };
  const notification = document.createElement('div');
  notification.style.cssText = `position: fixed; top: 20px; right: 20px; background: linear-gradient(135deg, #ffd700, #ff9500); color: #000; padding: 20px 30px; border-radius: 12px; font-weight: bold; font-family: 'Orbitron', sans-serif; box-shadow: 0 10px 30px rgba(255,215,0,0.5); z-index: 9999; animation: slideIn 0.5s ease;`;
